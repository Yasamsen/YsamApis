...
- **Auth:** IP-based auth 2.0 (Passport)
- **Deploy:** Vercel-ready
...
- Auto identify by IP → auto API key (`sk_...`)
- Daily limit (10 req/day) with auto UTC reset
- Admin role via `ADMIN_IPS` env (unlimited)
...
Set your IP in `ADMIN_IPS` (comma-separated) → automatic `role: admin`, unlimited requests, access to `/admin`.
